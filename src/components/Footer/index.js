        ...
        <div className={clsx(style.map)}>
          <YMaps>
            <Map width={359} height={177} defaultState={defaultState}>
              <Placemark geometry={[53.714198, 91.41745]} />
            </Map>
          </YMaps>
        </div>
        <div className={clsx(style.contacts)}>
          <div>
            <span className={clsx(style.title)}>Адрес:</span>
            <span className={clsx(style.subtitle)}>{contacts.address}</span>
          </div>
          <div>
            <span className={clsx(style.title)}>Телефон:</span>
            <Link
              className={clsx(style.subtitle)}
              to={`tel:${contacts.phone_number}`}
            >
              {" "}
              {contacts.phone_text}{" "}
            </Link>
          ...
          <div>
            <span className={clsx(style.title)}>E-mail:</span>
